...
def _get_downsampled_pc_torch(env, lidar, env_ids: torch.Tensor, max_pts: int | None):
    ...
    pc = lidar.get_pointcloud(env_ids)  # 可能是 (E,P,3) 或 (P,3) 或 (E,?,3)
    if pc is None:
        return None, None
    ...
    # 过滤 NaN/Inf：按点过滤
    finite_mask = torch.isfinite(pc).all(dim=-1)  # (E,P)
    # 为了保持张量形状固定，不做变长 gather（否则 batch 很难）
    # 我们用一个办法：把无效点设为 nan，然后后续在计算 r 时用 mask 去掉
    pc = pc.clone()
...
def obs_lidar_min_range_grid(
    ...
    """
    返回: (num_envs, T*P) 展平的一维lidar栅格最小距离观测
    - theta: 以 +z 轴为起点的极角 [0,180] 度
    - phi: 方位角 [0,360) 度
    - 每个 bin 存最小 r
    """
    # lidar 可能没启用：输出固定维度的 0
    # lidar 可能没启用：输出固定维度的 0
    ...
    try:
        lidar = env.scene[lidar_name]   # ✅ 用 try/except，而不是 "in env.scene"
    except KeyError:
        return torch.zeros(out_shape, device=env.device, dtype=torch.float32)
    except Exception:
        # 有些版本可能抛别的异常（更稳）
    ...
    # r
    r = torch.sqrt(x * x + y * y + z * z + 1e-12)  # (E,P)

    # theta = acos(z/r) in deg, 0 at +z
    cos_theta = torch.clamp(z / r, -1.0, 1.0)
    theta = torch.rad2deg(torch.acos(cos_theta))  # (E,P)

    # phi = atan2(y,x) in deg -> [0,360)
    ...
    if m.any():
        ...
        # 对每个 env 做 scatter_reduce amin
        # PyTorch 2.x 支持 scatter_reduce_
        for e in range(env.num_envs):
            me = m[e]
            if me.any():
                idx_e = lin_idx[e, me]          # (K,)
                r_e = r[e, me].to(torch.float32)  # (K,)
                bins[e].scatter_reduce_(0, idx_e, r_e, reduce="amin", include_self=True)

    # 把 inf 替换为 empty_value（表示该方向无点）
    if empty_value == 0.0:
...
# ✅ 新增：LiDAR   极角/方位角网格最小距离（展平后 4320 维）
        lidar_grid = ObsTerm(
            ...
            params={
                ...
                # 这里也可以复用你原先用于debug的 max_vis_points 作为“初步下采样上限”
