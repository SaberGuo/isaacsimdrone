...
TASK_ID_SCAN_LIDAR = "Isaac-OmniPerception-Drone-ScanLidar-v0"
...
gym.register(
    ...
    disable_env_checker=True,
    kwargs={
        # The policy LiDAR state is now generated from exact obstacle geometry,
        # so this task no longer instantiates the scan-based LiDAR sensor.
        "env_cfg_entry_point": "omniperception_isaacdrone.envs.test_env_cfg:Test6DroneEnvCfg",
    },
)

gym.register(
    id=TASK_ID_SCAN_LIDAR,
    entry_point=ENTRY_POINT,
    disable_env_checker=True,
    kwargs={
        # Legacy diagnostics that explicitly inspect LidarSensor.get_pointcloud().
