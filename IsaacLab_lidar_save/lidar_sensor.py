...
class LidarSensor(RayCaster):
    ...
    def _generate_pointcloud(self, env_ids: Sequence[int]):
        ...
        if self.cfg.pointcloud_in_world_frame:
            ...
            # Point cloud in TRUE sensor frame (Translation + Inverse Rotation)
            
            # 1. 获取传感器的世界位置和姿态
            sensor_pos = self._get_true_sensor_pos()[env_ids].unsqueeze(1)
            sensor_quat = self._data.quat_w[env_ids]  # Shape: (num_envs, 4) [w, x, y, z]
            
            hit_points_world = self._data.ray_hits_w[env_ids]
            
            # 2. 平移 (Translation)：消除位置差，将原点移到传感器中心
            translated_points = hit_points_world - sensor_pos
            
            # 3. 准备逆旋转 (Inverse Rotation)
            # 扩展四元数形状以匹配射线数量: (num_envs, num_rays, 4)
            sensor_quat_expanded = sensor_quat.unsqueeze(1).expand(-1, self.num_rays, -1)
            
            # 计算共轭四元数求逆 [w, x, y, z] -> [w, -x, -y, -z]
            sensor_quat_inv = sensor_quat_expanded.clone()
            sensor_quat_inv[..., 1:4] = -sensor_quat_inv[..., 1:4]
            
            # 4. 旋转 (Rotation)：应用逆四元数，把世界朝向扭转回机体朝向
            pointcloud_sensor = quat_apply(sensor_quat_inv, translated_points)
